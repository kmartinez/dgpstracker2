...
# between them)
MSG_PERIOD = 60  # 60-second debug mode
if MSG_PERIOD < 60:
    MSG_PERIOD = 60
...
def getReadings(i=0):
    ...
    chosen_msgs = []
    if LOG_MEDIAN:
        chosen_msgs.append(getMedianMsg(msg_buf))
        type_code = b'\x01'

    if LOG_BEST:
        # take message with smallest pAcc --> most accurate of the readings
        chosen_msgs.append(getBestAcc(msg_buf))
        type_code = b'\x02'

    if LOG_RAW:
        # take last message
        chosen_msgs.append(msg_buf[max(msg_buf)])
        type_code = b'\x00'

    if len(chosen_msgs) > 0:
        for msg in chosen_msgs:
            type_code = 0x00
            location = msg[LOC_CODE]
            log = BinaryECEFLog()
