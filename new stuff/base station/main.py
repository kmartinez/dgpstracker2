...
CALIBRATION_TTL = 50 # number of UART timeouts until calibration attepts stopped
...
def loadTimeParams(data):
    global MSG_PERIOD, MSG_START_TIME, TIME_CONF_LIMIT, UPDATE_DELAY
...
def calibrate():
    ...
    start_bit_two = False
    ttl = CALIBRATION_TTL
    while not (start_bit_one and start_bit_two) and ttl > 0:
        curr = gpsIn.read(1)
        if curr == b'\xb5':
            start_bit_one = True
        elif curr == b'\x62':
    ...
    while msg_count < (NO_READINGS + 1) * NO_MSGS + 1 and ttl > 0:
        ...
        if curepoch != lastepoch and lastepoch != 0:
            ...
            if any(map(lambda r: r is None, msg_buf[lastepoch])) or len(msg_buf[lastepoch]) < NO_MSGS:
                ...
                print(msg_buf[lastepoch])
                del msg_buf[lastepoch]
                lastepoch = 0
            elif LOG_RAW:
                # safe to log as raw data
                Log.ECEFLog(msg_buf[lastepoch][LOC_CODE], b'\xF1', msg_buf[lastepoch][SATINF_CODE]).writeLog()
