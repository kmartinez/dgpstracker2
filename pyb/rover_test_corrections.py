# Title: rover_test_corrections.py
# brief: validate that rover can received forwarded messages from base, write them to the gps_uart and then forward nmea
#        messages back to the base once they have been processed. - Will still test the code out in the field before
#        confirming it works.
...
while True:
    if radio_uart.any() > 0:
        ...
        if gpsFormatOutput(ROVER_ID, gps_rover_data)[0] == 'p':
            ...
            radio_uart.write(gpsFormatOutput(ROVER_ID, gps_rover_data))
